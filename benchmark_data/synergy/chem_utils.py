...
def drop_dup_rows(data):
    """ Drops duplicate rows, resets index, prints number of duplicates dropped. 
    
    Args:
        data (pd.DataFrame): DataFrame to drop duplicates from.

    Returns
        pd.DataFrame: DataFrame with duplicates dropped and index reset.
    """
    print('\nDrop duplicates ...')
    cnt0 = data.shape[0]
...
def dropna(df, axis: int=0, th: float=0.05, max_na: int=None):
    ...
    Args:
        df (pd.DataFrame): DataFrame to drop rows/cols from.
    ...
    Returns:
        pd.DataFrame: DataFrame with rows/cols dropped.
    """
...
def canon_single_smile(smi):
    """Canonicalize single SMILES string with RDKit. 
...
def fps_single_smile(smi, radius=1, nbits=2048):
    """ Convert single smiles into Morgan fingerprints with RDKit.
    ...
    Args:
        smi (str): A SMILES string.
        radius (int): Radius to use to calculate fingerprints (default: 1).
        nbits (int): Nbits to use to calculate fingerprints (default: 2048).
    
    Returns: 
        np.array: Fingerprint values for the SMILES string given.
    """
...
def canonicalize_smiles(smi_df, id_col_name='DrugID', smiles_col_name='smiles', par_jobs=1):
    """Canonicalize a dataframe containing SMILES strings with RDKit. 

    Args:
        smi_df (pd.DataFrame): A DataFrame containing two columns, one with the drug ID and one with the SMILES strings.
        id_col_name (str): Name of column containing drug IDs (default: 'DrugID').
        smiles_col_name (str): Name of column containing SMILES strings (default: 'smiles').
        par_jobs (int): Number of parallel jobs to run (default: 1).

    Returns:
        tuple[pd.DataFrame, pd.DataFrame]: A tuple containing:
            - pd.DataFrame: A dataframe with columns [id_col_name, smiles_col_name] containing the valid SMILES strings.
            - pd.DataFrame: A dataframe with columns [id_col_name, smiles_col_name] containing the invalid SMILES strings.
    ...
    # Add canonicalized SMILES string to dataframe
    can_smi_df = smi_df
...
def generate_fingerprints(smi_df, smiles_col_name='smiles', radius=1, nbits=2048, par_jobs=1):
    """Generate dataframe of fingerprints from a dataframe containing SMILES strings with RDKit. Should be used after 
    canonicalize_smiles with the valid (good) SMILES.

    Args:
        smi_df (pd.DataFrame): A DataFrame containing two columns, one with the drug ID and one with the valid SMILES strings.
        smiles_col_name (str): Name of column containing SMILES strings (default: 'smiles').
        radius (int): Radius to use to calculate fingerprints (default: 1).
        nbits (int): Nbits to use to calculate fingerprints (default: 2048).
        par_jobs (int): Number of parallel jobs to run (default: 1).
    ...
    Returns:
        pd.DataFrame: A dataframe with columns id_col_name and the fingerprint columns.
...
def generate_mordred(smi_df, smiles_col_name='smiles', ignore_3D=True, th = 0.25, par_jobs=1):
    """ Generate dataframe of Mordred descriptors from a dataframe containing SMILES strings with RDKit and Mordred. 
    Should be used after canonicalize_smiles with the valid (good) SMILES. Drops rows based on a threshold of NaN 
    values and imputes remaining missing values with 0.0.
    
    Args:
        smi_df (pd.DataFrame): A DataFrame containing two columns, one with the drug ID and one with the valid SMILES strings.
        smiles_col_name (str): Name of column containing SMILES strings (default: 'smiles').
        ignore_3D (bool): Whether to ignore (True) or use (False) the 3D descriptors (default: True).
        th (float): Threshold of fraction of NaN values to drop.
        par_jobs (int): Number of parallel jobs to run (default: 1).

    Returns:
        tuple[pd.DataFrame, pd.DataFrame]: A tuple containing:
            - pd.DataFrame: A dataframe with columns id_col_name and the descriptor columns with imputed values.
            - pd.DataFrame: A dataframe with columns id_col_name and the descriptor columns without imputed values.
    """
    from rdkit import Chem
...
def generate_infomax(smi_df, smiles_col_name='smiles'):
    """Generate dataframe of infomax fingerprints from a dataframe containing SMILES strings with the pretrained
    gin_supervised_infomax (see: https://github.com/NetPharMedGroup/publication_fingerprint). 
    Should be used after canonicalize_smiles with the valid (good) SMILES.
    ...
    Returns:
        pd.DataFrame: A dataframe with columns id_col_name and the infomax fingerprint columns.

    """
    # Generate infomax fingerprints 
    import torch
